...
push_release() {
  ...
  git commit -s -am "release: v$_version_no_tag"
...
open_pull_request() {
  ...
  curl -XPOST -H "Authorization: token $PAT_GITHUB" --data  "@$_body" \
...
bump_version() {
  ...
  git add VERSION
  git commit -s -m "ci: version bump to v$VERSION"
...
create_release() {
  ...
  generate_release_body "$_body" "true"
  curl -XPOST -H "Authorization: token $PAT_GITHUB" --data  "@$_body" \
  ...
  _release_url=$(cat $_release | jq -r '.url')
  ...
  for target in ${TARGETS[*]}; do
    ...
    curl -s -H "Authorization: token $PAT_GITHUB"    \
  ...
  log "uploading frontend-client artifact to GH release at ($_upload_url)"
    curl -s -H "Authorization: token $PAT_GITHUB"  \
        -H "Content-Type: application/gzip"        \
        --data-binary "@dist/frontend-client.tgz"  \
        "${_upload_url}?name=frontend-client.tgz"

  generate_release_body "$_body" "false"
  curl -XPOST -H "Authorization: token $PAT_GITHUB" --data  "@$_body" $_release_url > $_release
